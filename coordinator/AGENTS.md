...
| Skill | 什么时候用 | 路径 |
|-------|-----------|------|
| **butler-comm-skill** | 委托 Trip/Schedule/Account Agent 时，通过 shared/ 发消息 + cron run 唤醒（见 §6）| `coordinator/skills/butler-comm-skill/SKILL.md` |
| **subagent-skill** | 委托任务到子 agent 的高级封装 | `coordinator/skills/subagent-skill/SKILL.md` |
| **weather-monitor-skill** | trip-agent replan 回调带 weather_change=true → 拼天气提示文案 + 询问用户 | `coordinator/skills/weather-monitor-skill/SKILL.md` |
| **queue-monitor-skill** | trip-agent replan 回调带 queue_increase=true → 拼餐厅排队提示 | `coordinator/skills/queue-monitor-skill/SKILL.md` |
| **traffic-monitor-skill** | trip-agent replan 回调带 traffic_congestion=true → 拼交通拥堵提示 | `coordinator/skills/traffic-monitor-skill/SKILL.md` |
| **nearby-search-skill** | 用户问"附近有啥"→ 从 trip.json 拿位置 + query_nodes + 加权排序 | `coordinator/skills/nearby-search-skill/SKILL.md` |
| **replan-skill** | **不在 Coordinator 本地调** —— 是 trip-agent 内部 skill，trip 用 | `agents/trip-agent/skills/replan-skill/SKILL.md` |
| **memory-layers-skill** | 每次操作后写 transient，对话结束写 days | `skills/memory-layers-skill/SKILL.md` |
| **memory-seven-dim-skill** | 管理认知风格、健康情况两个维度 | `skills/memory-seven-dim-skill/SKILL.md` |
| **init-questionnaire** | 初始化问卷流程（HTML 在 `coordinator/scripts/user-init-questionnaire.html`）| HTML 静态文件 + `coordinator/memory/init-readme.md` |
