...
| Skill | 什么时候用 | 引用章节 |
|-------|-----------|---------|
| **trip-skill** | 收到 Coordinator 委托的行程规划任务 → 执行四阶段流程 | `agents/trip-agent/skills/trip-skill/SKILL.md` |
| **replan-skill** | 收到 mockend 坏事件后 → 调对应 monitor skill + 拼备选行程 | `agents/trip-agent/skills/replan-skill/SKILL.md` |
| **butler-comm-skill** | 接收 Coordinator 委托、回复结果、与其他 Agent 通信 | `coordinator/skills/butler-comm-skill/SKILL.md`「Agent 间通信」章节 |
| **memory-layers-skill** | 每次操作后写 transient，对话结束后写入 days | `skills/memory-layers-skill/SKILL.md`「操作后写 transient」+「对话结束写 days」 |
| **memory-seven-dim-skill** | 管理口味偏好维度（写 cache，≥3次晋升 dimension） | `skills/memory-seven-dim-skill/SKILL.md`「口味偏好维度 cache写入」+「cache → dimension 晋升流程」 |
...
| `agents/trip-agent/skills/trip-skill/temp/` | 读写 | 四阶段流程中间输出 |
...
```
event_detector 推送坏事件 → 调 replan-skill
  ↓
replan-skill 读 inbox.bad_events
  ↓
对每个 event 按 type 调对应 monitor skill：
  - type 2/3/4 (天气变化)   → coordinator/skills/weather-monitor-skill
  - type 5 (排队增加)       → coordinator/skills/queue-monitor-skill
  - type 10/11 (交通拥堵)   → coordinator/skills/traffic-monitor-skill
  - type 7/8/9 (POI/餐厅/道路)  → trip-agent 内部直接 replan
  ↓
调 phase3 重算备选路线
  ↓
拼成 result_callback 消息 → 写 shared/coordinator/YYYY-MM-DD.json
  ↓
